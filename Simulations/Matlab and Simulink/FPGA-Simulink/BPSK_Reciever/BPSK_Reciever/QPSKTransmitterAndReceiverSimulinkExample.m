...
% with Simulink(R). The receiver addresses practical issues in wireless
...
% and outputs a simple message to the Diagnostic Viewer. For the MATLAB(R)
...
% remaining bits are the payload. The payload corresponds to the ASCII
...
%% Frame Synchronizer
% The frame synchronization is performed by a MATLAB System block using a
% *FrameSynchronizer* System object(TM). The block uses the known frame
% header (QPSK-modulated Barker code) to correlate against the received
% QPSK symbols in order to find the location of the frame header. The block
% uses this location information to align the frame boundaries. It also
